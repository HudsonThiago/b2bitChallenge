  ...
  return (
    <div className="bg-bg1 w-full h-screen bg-bg1 flex items-center justify-center">
      <div className="lg:w-4/12 xl:w-3/12 sm:w-96 sm:m-6 box-border bg-white flex flex-grow sm:flex-grow-0 mx-4 flex-col items-center gap-8 shadow-2xl rounded-lg p-6 pt-16">
        ...
        {
          buttonPressed === false
          ? (<button onClick={()=>submitForm()} className={`${(email==="" || password==="")?'bg-slate-300':'bg-primary'} bg-primary w-full p-3 font-normal rounded-md select-none cursor-pointer`} disabled={(email==="" || password==="")?true:false}>
                <p className="text-white" >Sign In</p>
            </button>)
          : (<button className="bg-slate-300 w-full p-3 font-normal rounded-md select-none flex items-center justify-center gap-2 cursor-default" disabled>
                <img className="w-6 h-auto animate-spin" src={loading} alt="loading" />
                <p className="text-white mr-4" >Processing...</p>
            </button>)
        }
      </div>
      <div className="absolute top-0 left-auto sm:top-auto sm:bottom-0 sm:left-0 flex flex-grow flex-col sm:flex-grow-0 w-full hidden opacity-0">
